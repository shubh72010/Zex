...
const express = require('express'); const handler = require('./cmds'); const automod = require('./automod'); const verifier = require('./verifier');
...
new SlashCommandBuilder().setName('quote').setDescription('Send a random quote'), new SlashCommandBuilder().setName('flip').setDescription('Flip a coin'), new SlashCommandBuilder().setName('uptime').setDescription('Check bot uptime'), new SlashCommandBuilder().setName('status').setDescription('Bot status'),
...
const verifyChannel = client.channels.cache.get('1378653094312804392'); if (verifyChannel) { verifyChannel.send({ content: '📢 To enter Zeta HS, you must have a verified phone number and email linked to your Discord account.\n\nReverification is required every 14 days. You will be reminded 24 hours before expiry, and failure to reverify results in removal from the server.' }).catch(console.error); } });

client.on(Events.InteractionCreate, async interaction => { if (interaction.isChatInputCommand()) { handler(interaction); } });

automod(client); verifier(client);
